...
SIM bude adapter/cache mezi TAK/ARDOS světem a COP. COP neparsuje CoT XML a nenapojuje se přímo na TAK server. COP čte pouze stabilní SIM endpoint:

```text
GET /tak-gateway/api/v1/cop/features
```
...
- COP musí rozhodovat viditelnost vrstvy podle role uživatele.
- SIM ani COP nesmí nad daty implementovat targeting, navádění nebo zbraňové workflow.
...
3. Po dohodě s COP doplnit interní oprávnění pro neveřejnou vrstvu.
