...
Produkčně platí, že COP je jediný frontend. SIM provider endpointy nejsou veřejné UI API a mají být dostupné pouze server-to-server z COP backendu, interních sítí nebo VPN. Veřejně smí zůstat jen `GET /health/live` a dokumentační stránka. Detaily Nginx allowlistu jsou v [../runbooks/12_PROVIDER_ACCESS_CONTROL.md](../runbooks/12_PROVIDER_ACCESS_CONTROL.md).
