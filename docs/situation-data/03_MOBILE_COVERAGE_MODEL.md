...
```env
SITUATION_DATA_ENABLED_SOURCES=open_meteo,aviation_weather,chmi_weather_stations,chmi_air_quality,osm_postgis,mobile_coverage_model,mobile_network_model,ctu_nettest,ctu_stationary_mobile,pid_gtfs_rt,spravazeleznic_trains,road_srti_lod,safety_data
...
- The read-model keeps parsed grid metadata (`grid_resolution_m`, `grid_row`, `grid_column`) and cached cell bounds (`bbox_west`, `bbox_south`, `bbox_east`, `bbox_north`) beside the polygon geometry. Runtime sampling uses `mobile_coverage_cells_candidate_idx` and `mobile_coverage_cells_lookup_idx` over narrow metadata rows, joins back to full polygons only after the final limit and serializes only that limited set to GeoJSON; this avoids GiST full-bbox scans, ranking wide JSON payloads or converting the full national grid on every low-zoom request. The read-model lookup raises local `work_mem` only for this query so national low-zoom sampling stays in memory under normal pilot limits.
