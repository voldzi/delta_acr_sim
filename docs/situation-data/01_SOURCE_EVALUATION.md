...
| `traffic` | Veřejné statické GTFS feedy | implementováno jako `public_transit_static`; výchozí ověřený feed je PID statický GTFS a další města/regiony se přidávají konfiguračně přes `PUBLIC_TRANSIT_STATIC_GTFS_FEEDS` | statický zastávkový kontext, nikoli live poloha; SIM cacheuje ZIP server-side a COP nesmí volat upstream přímo |
