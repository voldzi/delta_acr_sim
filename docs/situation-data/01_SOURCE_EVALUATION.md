...
| `ground` | OpenStreetMap přes lokální OSM extract/PostGIS (`osm_postgis`); Overpass jen pro vývoj | implementováno jako lokální PostGIS provider pro referenční POI po importu Geofabrik PBF extractu; veřejný Overpass pouze pro malé testovací bbox dotazy | ODbL 1.0, atribuce a share-alike povinnosti pro databáze; veřejné Overpass instance nejsou produkční runtime backend |
