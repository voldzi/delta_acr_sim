...
| `traffic` | Veřejné statické GTFS/GeoJSON feedy | implementováno jako `public_transit_static`; výchozí ověřená sada je PID, IDS JMK, DPMO Olomouc, PMDP Plzeň, DPMLJ Liberec/Jablonec a DPO Ostrava GeoJSON; další města/regiony se přidávají konfiguračně přes `PUBLIC_TRANSIT_STATIC_GTFS_FEEDS` a `PUBLIC_TRANSIT_STATIC_GEOJSON_FEEDS` | statický zastávkový kontext, nikoli live poloha; SIM cacheuje ZIP server-side a COP nesmí volat upstream přímo |
...
| `outdoor_webcams` | Kurátorované originální weby provozovatelů kamer z českého WebCamLive discovery auditu | implementováno jako samostatná vrstva `public.outdoor.webcams` nad source `chmi_weather_webcams`; vestavěný feed obsahuje 267 dostupných originálních provider stránek bez WebCamLive image proxy | patří do Turistika / Outdoor; přímý snapshot je vypnutý, dokud není ověřená přímá originální image/stream URL |
