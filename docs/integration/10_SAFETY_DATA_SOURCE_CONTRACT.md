...
- `source=chmi_alerts,chmi_hydro,nasa_firms,gdacs_alerts,hzs_incidents,municipal_alerts,road_srti_lod,admin_boundaries` nebo `source=mock`.
- `limit=1..1000`.
...
- `gdacs_alerts`: GDACS veřejné RSS/GeoRSS katastrofické alerty z `https://www.gdacs.org/xml/rss.xml`; bez klíče. `FL` se promítá do `flood`, `WF` do `fire`, ostatní typy jako `EQ`, `TC`, `VO`, `DR` do `warnings`. Pokud COP požádá o `warnings`, SIM vrací i obecnou kopii GDACS požáru/povodně jako krizový alert. SIM lokalizuje GDACS primárně podle `geo:Point`/`georss:point`; `gdacs:bbox` používá jako náhradní mapový bod jen pro malé přesné bboxy. Hrubé státní nebo nadregionální GDACS bboxy se nepromítají do středu dotazovaného bboxu COP.
- `hzs_incidents`: veřejné probíhající výjezdy HZS/IZS. Výchozí pilotní katalog obsahuje HZS Pardubického kraje z oficiální HTML stránky HZS ČR a veřejné JSON API HZS Královéhradeckého kraje. U HTML feedu SIM bere pouze sekci `Probíhající výjezdy` a detail události používá pro typ, podtyp, stav, obec, část obce, ulici a jednotky. U KHK JSON feedu SIM bere jen průběžné stavy událostí (`Převzatá`, `Otevřená`, `Lokalizovaná`, `Likvidovaná`), ne ukončené denní záznamy, a souřadnice `gis1/gis2` převádí ze S-JTSK/Křováka do WGS84 s `properties.tags.locationPrecision=source_point`. Požáry se projektují do `fire` i do obecné `warnings`, ostatní typy do `warnings` s jasným `hazardType` (`traffic_accident`, `hazmat`, `rescue`, `technical_assistance`, `false_alarm`, `emergency_incident`). Pokud detail neobsahuje přesnou GPS, SIM geokóduje přes lokální `public.osm_admin_boundary` a v `properties.tags.locationPrecision`/`properties.metrics.locationConfidence` označí, zda jde o centroid obce/správního území nebo fallback kraje.
- `municipal_alerts`: konfigurované krajské a městské krizové feedy ve formátu RSS/Atom/GeoRSS/GeoJSON nebo veřejné PKR JSON výstupy. Pokud je zdroj zapnutý a `MUNICIPAL_ALERT_FEEDS` je prázdné, SIM použije vestavěný ověřený katalog veřejných regionálních zdrojů (`pkr-ustecky-jpo`, `pkr-liberecky-udalosti`, `pkr-stredocesky-aktuality`, `pkr-stredocesky-jpo`, `olkraj-krizove-rizeni`, `bruntal-uredni-rss`, `krnov-aktuality-rss`, `vrbno-aktuality-rss`). SIM je publikuje do `public.safety.warnings` jako normalizované krizové výstrahy s `sourceSystem=MUNICIPAL_CRISIS_FEED`, `typeCode=municipal.*` a `providerProperties.schemaVersion=sim.municipal-alerts.v1`. PKR JSON body jsou převáděné ze S-JTSK/Křováka do WGS84 a mají `properties.tags.locationPrecision=source_pkr_json`. Pokud feed nedodá přesnou geometrii ani bod, SIM použije konfigurovaný bod autority s `properties.tags.locationPrecision=authority_fallback_point` a sníženou důvěrou; COP to má v detailu jasně označit jako nepřesnou polohu zdroje, ne polohu události. Obecné městské RSS feedy v katalogu jsou pro COP přísně filtrované na krizově relevantní texty, aby se do výstrah nepromítaly běžné aktuality nebo úřední deska.
- `road_srti_lod`: veřejné NDIC/ŘSD SRTI dopravní události z TamTam Research Linked Open Data SPARQL. SIM je v `safety-data` promítá pouze do `warnings` jako normalizované dopravně-bezpečnostní prvky s `hazardType=road_incident`, kanonickým `typeCode` (`road.accident`, `road.closure`, `road.obstruction`, `road.weather_condition`, `road.abnormal_traffic`, `road.roadworks`), českým/anglickým `properties.localized` a hotovým `headline`/`description`/`recommendedAction`. Plná dopravní kontextová vrstva zůstává v `situation-data` jako `public.traffic.road_events`.
...
```text
GET /situation-data/api/v1/features?layers=warnings,weather_alerts,fire,flood,boundary_admin&source=safety_data
```
...
- `safety_data_source_cache_hits/misses/stale_hits/errors{source="chmi_alerts|chmi_hydro|nasa_firms|gdacs_alerts|hzs_incidents|municipal_alerts|road_srti_lod|admin_boundaries"}`,
