...
- `municipal_alerts`: konfigurované krajské a městské krizové feedy ve formátu RSS/Atom/GeoRSS/GeoJSON. SIM je publikuje do `public.safety.warnings` jako normalizované krizové výstrahy s `sourceSystem=MUNICIPAL_CRISIS_FEED`, `typeCode=municipal.*` a `providerProperties.schemaVersion=sim.municipal-alerts.v1`. Pokud feed nedodá přesnou geometrii ani bod, SIM použije konfigurovaný bod autority s `properties.tags.locationPrecision=authority_fallback_point` a sníženou důvěrou; COP to má v detailu jasně označit jako nepřesnou polohu zdroje, ne polohu události.
...
- `municipal_alerts` drží per-feed cache podle `MUNICIPAL_ALERTS_CACHE_TTL_SECONDS` a dotazuje jen feedy, jejichž konfigurovaný bbox protíná query bbox.
