...
| `pid_gtfs_rt`              | `traffic`                                                                                                                                        | PID/Golemio GTFS-RT vozidla pro dopravní kontext. Výchozí zdrojová i publikovaná kadence je 15 s.                                                                                                                                                                                                                                        |
...
Agregační odpověďová cache používá pro dotazy s živou dopravou minimum z
obecného `SITUATION_DATA_CACHE_TTL_SECONDS` a TTL vybraných živých providerů.
PID proto není zadržován obecnou 30sekundovou cache. Source-level cache a
stale-if-error zůstávají aktivní, takže více klientů nezvyšuje počet volání
Golemio nebo NDIC úměrně počtu uživatelů.
