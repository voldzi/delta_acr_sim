...
- ČHMÚ 10min station feed neposkytuje pro každou feature autoritativní stav oblohy typu `jasno/polojasno/oblačno`. SIM proto u `weather.chmi_station_observations` posílá explicitní prezentační hint `providerProperties.weather.symbolKey`, `providerProperties.weather.conditionLabel`, `providerProperties.weather.authoritativeCondition` a `providerProperties.presentation.mapLabel`. COP má tyto hodnoty použít před vlastní inferencí a nesmí při chybějící oblačnosti automaticky zobrazit `polojasno`.
- Konzervativní mapování SIM: naměřené srážky dávají `rain` nebo při nízké teplotě `snow`, silný naměřený vítr dává `wind`, velmi vysoká vlhkost se slabým větrem dává nízkodůvěrové `fog`, silný sluneční svit dává `sun`; jinak je symbol `measurement` a label `měřené počasí`.
- ČHMÚ publikuje i hodinové `1h-*` soubory s prvky jako `N`, `ww`, `W1`, `W2`, `VV` a ceilometrové veličiny pro část stanic. Ty jsou vhodné pro následný enrichment oblačnosti/stavu počasí, ale základní COP rendering nesmí na jejich absenci vynucovat falešný stav oblohy.
