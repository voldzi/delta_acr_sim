...
- `public.weather.forecast_area` / provider layer `weather.forecast_area`: plošné předpovědní polygony ze zdroje `weather_forecast`. Každá feature nese `providerProperties.presentation.symbolKey`, `conditionLabel`, `riskLevel`, `colorRamp`, `metrics.riskScore`, `metrics.precipitationNext3hMm`, `metrics.precipitationProbabilityNext3hPercent`, `metrics.windSpeedMps`, `metrics.windGustMps` a `providerProperties.weatherForecast.detailUrl`. COP nemá z `weatherCode` sám odvozovat ikonu; má vykreslit dodaný `symbolKey` a barvit polygon podle `riskScore`/`riskLevel`.
- Detail předpovědní oblasti je `GET /weather-forecast/areas/{areaId}?bbox=west,south,east,north&hours=48&days=7`. Vrací `contractVersion=sim-weather-forecast-area-detail-v1`, `summary`, `current`, `nowcast.points`, `hourly.points`, `daily.points` a hotové `charts[]` pro teplotu, srážky, vítr a riziko. COP má grafy pouze vykreslit podle `charts[].series[].points`.
...
GET /weather-forecast/areas/{areaId}?bbox=14.0,49.8,14.25,50.05&hours=48&days=7
...
| `public.weather.forecast_area` | `weather.forecast_area` | `vector_features` | SIM předpovědní agregát nad Open-Meteo |
